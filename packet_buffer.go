...
var (
	...
	ErrIsNotEmpty = errors.New("Buffer is not empty")

	// ErrAcquireLock is returned when the lock is not acquired on Try operations.
	ErrAcquireLock = errors.New("Unable to get lock")
...
// New returns a new RingBuffer whose buffer has the given size in bytes
...
// set error for ringbuffer structure
...
// read and return error from underlaying buffer field
